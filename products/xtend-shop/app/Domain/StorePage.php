...
final class StorePage {
    public function render(Request $r,string $view,array $extra=[]){
        $draft=app(Checkout::class)->draft($r);$catalog=app(Catalog::class);$data=[
            'id'=>'store','view'=>$view,'home'=>$view==='home','results'=>$view==='results','detail'=>$view==='detail','cartPage'=>$view==='cart','checkout'=>$view==='checkout','confirmation'=>$view==='order',
            'catalog'=>$catalog->search($r->query()),'product'=>['id'=>'','name'=>'','priceText'=>'','sku'=>'','variants'=>[],'features'=>[],'image'=>'','description'=>'','stockText'=>'','available'=>false],
            'cart'=>app(Cart::class)->read($r,$draft['shipping']),'csrf'=>$r->session()->token(),'draft'=>$draft,'order'=>['id'=>'','lines'=>[],'totalText'=>''],
        ];$data=array_replace($data,$extra);
        $title=$view==='detail'?$data['product']['name']:['home'=>'Gute Dinge für deinen Alltag','results'=>$data['catalog']['title'],'cart'=>'Dein Warenkorb','checkout'=>'Sicher zur Demo-Bestellung','order'=>'Danke für deine Demo-Bestellung'][$view];
        ...
        foreach(['name'=>'Vollständiger Name','email'=>'E-Mail-Adresse','street'=>'Straße und Hausnummer','postal'=>'Postleitzahl','city'=>'Ort','shipping'=>'Versandart'] as $field=>$label)$props['shop.'.$field]=['id'=>'checkout-'.$field,'field'=>$field,'label'=>$label,'value'=>$draft[$field]??'','inputType'=>$field==='email'?'email':'text','required'=>true];
        $props['shop.recommendations']=Prop::defer(fn()=>['items'=>array_slice($catalog->search([])['products'],4,4)],'recommendations');
