...
Dieser Handoff beschreibt den ersten lokalen RC1-Test-Build-Schnitt fuer XTend. Er ist als reproduzierbarer Owner-Review-Stand gedacht, nicht als npm Release. Das Paket bleibt in `package.json` mit `private: true` geschlossen; `publishAllowed` bleibt `false`.
...
- Conditional Network Evidence mit Owner-Deferral fuer netzwerkpflichtige Audit-/SBOM-Laeufe
...
| `.xtend-test-results/xtend-package-export-lock-report.json` | `ok: true`, `exportCount: 115`, `packFileCount: 658` |
| `.xtend-test-results/xtend-conditional-network-evidence-report.json` | `ok: true`, `publishAllowed: false`, Owner-Deferrals fuer `npm-audit-moderate` und `npm-sbom-json` |
...
- Ob die formal deferierten Netzwerklaeufe `npm audit --audit-level=moderate --json` und `npm sbom --json` vor einem echten Publish ausgefuehrt werden oder weiterhin blockierend bleiben.
...
- Ob und wann `private: true` in einem separaten Release-Owner-Schritt aufgehoben werden darf.
...
Die vorbereitete Acceptance liegt in `development/XTend-RC1-Test-Build-Owner-Acceptance.md` unter `xtend.rc1.test-build-owner-acceptance.v1`. Sie akzeptiert den Test-Build fuer interne Nutzung, laesst Audit-/SBOM-Deferrals publish-blocking und haelt `private: true` geschlossen.
