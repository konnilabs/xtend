...
type XToggleStateSource = 'attribute' | 'property' | 'user' | 'reset';
...
class XToggle extends HTMLElement {
  ...
  static get xtendFormControlUxProfile(): XToggleFormControlUxProfile {
    ...
    } as const as XToggleFormControlUxProfile;
  }
  ...
  constructor() {
    ...
    this.labelText = this.shadowRoot!.querySelector('#label-text') as HTMLElement;
    this.stateText = this.shadowRoot!.querySelector('#state-status') as HTMLElement;
  ...
  connectedCallback(): void {
    if (!this.id) this.id = `xtoggle-${Math.random().toString(36).slice(2, 10)}`;
  ...
  attributeChangedCallback(name: string, oldValue: string | null, newValue: string | null): void {
    ...
    if (name === 'label') {
      ...
      if (newValue) this.control.setAttribute('aria-label', newValue);
  ...
  private upgradeAttributes(): void {
  ...
  private setChecked(value: boolean, options: { emit: boolean; source: XToggleStateSource }): void {
  ...
  private handleInvalid(): void {
    ...
    this.dispatchEvent(new CustomEvent<XToggleInvalidEventDetail>('toggle-invalid', {
      detail: {
  ...
  private emitChanged(): void {
    this.dispatchEvent(new CustomEvent<XToggleChangedEventDetail>('toggle-changed', {
  ...
  private syncFormValue(): void {
    ...
    if (this.hasAttribute('required') && !this.checked) {
      ...
      if (!this.hasAttribute('invalid')) this.control.setAttribute('aria-invalid', 'false');
      else this.control.setAttribute('aria-invalid', 'true');
      if (this.checked || !this.hasAttribute('required')) this.removeAttribute('invalid');
      this.internalsRef?.setValidity?.({});
    }
    this.internalsRef?.setFormValue(this.checked ? this.value : null);
  }

  private publishState(source: XToggleStateSource): void {
    const state = {
