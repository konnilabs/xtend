...
class XCheckbox extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['name', 'value', 'checked', 'disabled', 'required', 'indeterminate', 'label', 'busy', 'invalid', 'density'];
  }
  ...
  static get xtendFormControlUxProfile() {
    return {
      ...
      signatureDesign: {
        note: 'Tactile enterprise checkbox with native reliability, explicit helper/error rhythm and density-safe alignment.',
        tokenStrategy: 'form tokens map label, control, helper, error, icon, focus, disabled, busy and density states.',
        themeExpectation: 'host applications can restyle the selection glyph, label and validation surfaces independently.'
      },
      densityProfiles: ['comfortable', 'compact', 'dense'],
      states: ['required', 'disabled', 'busy', 'invalid', 'indeterminate'],
    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          --xtend-form-radius: var(--xtend-control-radius, 0.25rem);
        }
      ...
      <label id="label" for="control">
        <input id="control" part="control icon" type="checkbox" aria-describedby="hint error">
      ...
      <div id="error" class="error" part="error status" role="alert" aria-live="assertive" aria-atomic="true"><slot name="error">This checkbox is required.</slot></div>
    `;
