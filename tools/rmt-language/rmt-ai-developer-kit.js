...
const RMT_AI_DEVELOPER_KIT_OUTPUT_DIR = 'tools/rmt-language/generated/rmt-ai-developer-kit';
...
const FIXED_SOURCE_PATHS = Object.freeze([
  ...
  'xtend-maraca/fastpass-contract.js',
  'xtend-maraca/fastpass.schema.json',
...
const REFERENCE_RECORDS = Object.freeze([
  ...
  operatorRecord('bounds', 'bounds mode responsive scope viewport x "1rem" y "1rem" width "clamp(20rem, 70vi, 52rem)" height "min(80dvh, 42rem)"', ['surface'], 'Declares fixed pixel or opt-in CSS-native responsive surface geometry.', 'bounds mode responsive scope viewport x "1rem" y "1rem" width "clamp(20rem, 70vi, 52rem)" height "min(80dvh, 42rem)" minWidth "18rem" maxHeight "48rem"', 'bounds mode responsive width clamp(20rem, 70vi, 52rem)', 'Responsive CSS values must be quoted; fixed bounds stay numeric pixels.', ['tools/rmt-language/vnext-compiler.js', 'tests/rmt-language/fixtures/vnext-responsive-bounds-valid.rmt']),
  ...
  operatorRecord('execution fastpass', 'action shell.navigate { execution fastpass effect navigation "#reports" }', ['action'], 'Runs urgent navigation, focus or named-surface close on the shared user-blocking lane without a Model transaction.', 'action shell.close { execution fastpass effect close surface panel }', 'action shell.close { execution fastpass reduce state.app.closed = true }', 'Only navigation, effect focus surface and effect close surface are allowed. Targets must exist. No reducers, status/results, fetch, services, streams, custom effects or chained business commands.', ['docs/de/rmt-reference-actions-events.md', 'docs/en/maraca-fastpass-abort-boundary.md', 'xtend-maraca/fastpass-contract.js']),
