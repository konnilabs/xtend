...
async function assertTrustedDomLocalServerProbe(context, rootDir) {
  ...
  try {
    ...
    context.fail(`Trusted DOM boundary local server probe (${error.message})`);
  } finally {
...
async function runEpic13TrustedDomBoundarySuite(options = {}) {
  ...
  context.assert(packageManifest.exports['./catalog/epic13-trusted-dom-boundary'] === './catalog/epic13-trusted-dom-boundary.js', 'Package exports Trusted DOM boundary module');
  ...
  context.assert(packageLockMetadata && packageLockMetadata.expectedExportCount === 104, 'Package export lock includes Trusted DOM boundary export');
