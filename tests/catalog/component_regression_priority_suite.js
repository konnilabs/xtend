...
function runComponentRegressionPrioritySuite(options = {}) {
  ...
  context.assert(plan.entries.length === 41, 'Plan covers all 41 Manifest components');
  context.assert(plan.summary.componentCount === 41, 'Summary counts all Manifest components');
  ...
  context.assert(xTheme && xTheme.catalogStatus === 'enterprise-ready', 'x-theme is enterprise-ready after WP-E12-05');
  ...
  context.assert(xButton && xButton.catalogStatus === 'enterprise-ready', 'x-button is enterprise-ready after WP-E12-06');
  context.assert(xButton && xButton.browserSmokes.includes('keyboard-activation'), 'x-button keeps interactive keyboard smoke');
  context.assert(xIcon && xIcon.catalogStatus === 'enterprise-ready', 'x-icon is enterprise-ready as iconography adapter');
  ...
  context.assert(xMenu && xMenu.catalogStatus === 'enterprise-ready', 'x-menu is enterprise-ready after WP-E12-07');
  ...
  context.assert(xState && xState.catalogStatus === 'contract-gated', 'xstate is contract-gated as a boundary probe after WP-E12-08');
  ...
  context.assert(xUtils && xUtils.tier === 'p1-visual-performance', 'x-utils moves to performance-focused typed gate after WP-E12-09');
