...
import {Prop, encodePageWire, decodePageWire, type PageManifest, type PageResponse, pagePagination} from '@ccslabs/xtend-rmt/page-contract';
...
const host=createNodePageHost({manifest,compactResponses:true,createContext:()=>({contextKey:'guest',account:1}),resolvePage:context=>({page:'Home',props:{title:Prop.once(async()=>String(context.account))}}),validate:()=>({errors:{}})});
host.dispose();
...
decodePageWire(encodePageWire(initialPage));
