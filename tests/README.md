...
The active CI workflow is `.github/workflows/xtend-default-gates.yml`. PR and release owners execute `npm run test:ci-pr` and `npm run test:ci-release` once per Node version. These catalog profiles include the corresponding `test:pr:report` / `test:release:full:report` obligations and the RMT/Native-First subset. The existing `rmt-vnext-primitive-gates` check verifies same-run execution artifacts and projects its reports. Node `24.18.0` and `26.5.0`, npm `11.17.0`, all OS matrices and artifact names remain unchanged. Docs stub inventory is advisory in PR/release CI and blocking in Nightly. Source-to-Sea remains optional via `workflow_dispatch` with `run_source_to_sea=true`. See [PR gate execution](../docs/en/xtend-dev-surface.md#pr-gates-and-shared-test-execution) for profiles, report contracts and verification.
