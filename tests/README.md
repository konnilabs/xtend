...
Run the optional RMT vNext Source-to-Sea browser evidence gate:
...
The active CI default gate is `.github/workflows/xtend-default-gates.yml` under `xtend.ci.default-gates.v1`. Since `ER-WP-37`, the workflow also follows `xtend.ci.gate-matrix.v1`: pull requests run `npm run test:pr:report` and upload `xtend-pr-gate-report-${{ matrix.artifact_suffix }}`; push and manual release checks use `npm run test:release:full:report` and upload `xtend-release-gate-report-${{ matrix.artifact_suffix }}`. Stage A runs both the primary Node `24.18.0` lane (`node-24-18-0`) and the mandatory Node `26.5.0` lane (`node-26-5-0`), pins npm `11.17.0`, and uploads `.xtend-test-results/runtime/xtend-node-runtime-${{ matrix.runtime_lane }}.json` with every matrix artifact. Package metadata keeps the provider-neutral `{artifactSuffix}` spelling. Both aggregate gates include `docs-stub-inventory` and `docs-content-depth`; CI also writes `.xtend-test-results/xtend-docs-stub-inventory-report.json` as a dedicated artifact so short visible guide articles fail with focused evidence. React/Vue XTensions adapter hardening runs through `npm run test:xtensions-framework-adapters:report` and uploads `.xtend-test-results/xtend-xtensions-framework-adapters-report.json` for host-provided runtime, PoC-compatibility and Maraca manifest drift evidence. RMT vNext Primitive changes additionally run `npm run test:rmt-vnext-primitives:report` in the `rmt-vnext-primitive-gates` job and upload `xtend-rmt-vnext-primitives-gate-report-${{ matrix.artifact_suffix }}`. Source-to-Sea browser evidence is optional and only runs in GitHub Actions through manual `workflow_dispatch` with `run_source_to_sea=true`. The PR and RMT primitive aggregates now include the PHP SSR adapter, Docs PHP SSR prehydration/performance/CLS budgets and the shared XTend layout-stability contract, so SSR and CLS regressions fail before release.

Nightly builds run through `.github/workflows/xtend-nightly-build.yml` under `xtend.ci.nightly-build.v1`. The scheduled Node `24.18.0`/`26.5.0` matrix writes `xtend-nightly-build-${{ matrix.artifact_suffix }}` with per-lane Runtime Evidence, `.xtend-test-results/xtend-nightly-build-manifest.json`, full release gate evidence, docs stub inventory evidence, RMT vNext primitive evidence, React/Vue XTensions framework adapter evidence, release report evidence and package dry-run evidence. The browser-heavy Source-to-Sea evidence and conditional network audit/SBOM evidence stay separate manual `workflow_dispatch` jobs behind `run_source_to_sea=true` and `run_conditional_network=true` and use the same matrix suffixes. The owner-approved npm publish job deliberately runs only on Node `24.18.0` and uploads `xtend-npm-publish-latest-evidence-node-24-18-0`.
