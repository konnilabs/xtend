...
function runDocsRmtPilotSuite(options = {}) {
  ...
  context.assert(indexPhp.includes('src="utils/pageloader.js?v='), 'Docs app loads the page loader with cache busting');
  ...
  context.assert(indexPhp.includes("'--input-bg-dark': '#0f0f12'"), 'Docs app dark theme exposes black-weighted x-input dark background token');
  context.assert(indexPhp.includes("'--input-placeholder-color-dark': '#a1a1aa'"), 'Docs app dark theme exposes x-input placeholder token');
  context.assert(indexPhp.includes("'--xtend-surface': '#0b0b0d'"), 'Docs app dark theme overrides canonical XTend surface token');
  context.assert(indexPhp.includes("'--xtend-text': '#f4f4f5'"), 'Docs app dark theme overrides canonical XTend text token');
  context.assert(indexPhp.includes("'--xtend-overlay-bg': 'rgba(0, 0, 0, 0.72)'"), 'Docs app dark theme overrides canonical XTend overlay token');
  context.assert(indexPhp.includes('x-code,x-modal,x-dialog'), 'Docs app preloads hands-on demo components');
  context.assert(indexPhp.includes("--body-bg': '#050506'"), 'Docs app dark theme uses a black-weighted body background');
  ...
  context.assert(pageLoader.includes('getDocsMenuEntryIcon'), 'Docs page loader maps every menu entry to an icon');
  context.assert(pageLoader.includes('docs-menu-link-icon'), 'Docs page loader renders x-icon elements inside routed menu links');
  context.assert(!/getDocsMenuGroupIcon\(group\.id\)\);\s*icon\.setAttribute\('pack', 'lucide'\)/u.test(pageLoader), 'Docs menu group icons resolve through the icon registry instead of forcing one pack');
  context.assert(indexPhp.includes('docs-menu-disclosure-icon'), 'Docs app styles Deep Dive menu entries as compact disclosure submenus');
  context.assert(indexPhp.includes('.docs-menu-children:not([open]) > .docs-menu-child-list'), 'Docs app hides Deep Dive submenu content while collapsed');
  context.assert(pageLoader.includes('closeSiblingDocsSubmenus'), 'Docs page loader keeps Deep Dive submenus accordion-like instead of leaving long containers open');
  ...
  context.assert(parsedownDocs.includes('docs.app.shell'), 'Parsedown scheduling docs document the RMT app shell');
  context.assert(parsedownDocs.includes('Shell-first'), 'Parsedown scheduling docs document shell-first rendering');
  context.assert(parsedownDocs.includes('node scripts/run_xtend_tests.js docs-rmt-pilot --json'), 'Parsedown scheduling docs document the pilot gate');
