...
function runDocsRmtPilotSuite(options = {}) {
  ...
  const pageEntry = readText('docs/utils/page/index.mjs', rootDir);
  const routeController = readText('docs/utils/page/route-controller.mjs', rootDir);
  const playgroundIsland = readText('docs/utils/page/playground-island.mjs', rootDir);
  ...
  context.assert(pageEntry.includes("export * from './route-controller.mjs'"), 'Docs page entry delegates to the route/resume controller');
  context.assert(routeController.includes("import('./playground-island.mjs')"), 'Playground is a route-local dynamically loaded island');
  context.assert(routeController.includes('scheduleDocsVisibleOrIntentIsland'), 'Deferred demos use the visible-or-intent island contract');
  context.assert(playgroundIsland.includes('export function createPlaygroundIsland') && playgroundIsland.includes('dispose()'), 'Playground island exposes a factory and idempotent disposer');
  ...
  context.assert(indexPhp.includes("import' => '/docs/utils/page/index.mjs?v='"), 'Docs app routes import the versioned ESM page entry module');
  context.assert(indexPhp.includes('src="/docs/utils/page/index.mjs?v='), 'Docs app loads the ESM page entry with cache busting');
  ...
  context.assert(!indexPhp.includes('x-code,x-modal,x-dialog') && routeController.includes('scheduleDocsVisibleOrIntentIsland'), 'Docs app keeps hands-on demo components out of the initial preload path');
  ...
  context.assert(routeController.includes(DOCS_RMT_RENDER_SCHEMA), 'Docs page loader emits RMT render metadata');
  ...
  context.assert(routeController.includes('export function createRouteController'), 'Docs page route module exposes an explicit controller factory');
  context.assert(routeController.includes('showDocsSkeleton'), 'Docs page loader uses the framework SkeletonLoader for Parsedown content');
  context.assert(routeController.includes('loadDocsParsedownContent'), 'Docs page loader lazy-loads Parsedown payloads per route');
  context.assert(routeController.includes('xtend.loader.skeleton-loader.v1'), 'Docs page loader records the native SkeletonLoader contract');
  ...
  context.assert(routeController.includes('renderDocsRelatedSidebar'), 'Docs page loader moves read-further links into the RMT sidebar');
  context.assert(routeController.includes('isDocsTrustedDomUrlAllowed(href)'), 'Docs page loader preserves routable and safe related links in the sidebar');
  context.assert(routeController.includes('missing-sidebar-slots'), 'Docs page loader falls back to a complete shell when sidebar slots are missing');
  context.assert(routeController.includes('ensureDocsShellScopedStyles(this.getRootNode())'), 'Docs page loader injects shell styles into the XRouter shadow route root');
  context.assert(routeController.includes('DOCS_SHELL_SHADOW_STYLE_ID'), 'Docs page loader de-duplicates scoped shadow styles');
  ...
  context.assert(routeController.includes('var(--docs-layout-gap'), 'Docs shadow-scoped shell styles use the shared layout gap token');
  context.assert(routeController.includes('var(--docs-sidebar-width'), 'Docs shadow-scoped shell styles use the shared sidebar width token');
  context.assert(routeController.includes('margin-inline: var(--docs-viewport-gutter, 0.5rem);'), 'Docs shadow-scoped shell styles keep article/sidebar inside the shared viewport gutter');
  context.assert(routeController.includes('#md-content {\n    min-width: 0;\n    max-width: 100%;'), 'Docs page loader keeps rendered Markdown content viewport-bound');
  context.assert(routeController.includes('x-section.docs-app-shell::part(container)'), 'Docs shadow-scoped shell styles widen x-section shell parts');
  context.assert(!/\.docs-page-sidebar\s*\{[^}]*position:\s*sticky/u.test(routeController), 'Docs shadow-scoped sidebar scrolls with the page instead of sticking to the viewport');
  context.assert(routeController.includes('renderDocsComponentDemo'), 'Docs page loader renders hands-on component demos');
  context.assert(routeController.includes('DOCS_COMPONENT_DEMOS'), 'Docs page loader declares component demo registry');
  context.assert(routeController.includes("getDocsAssetUrl('lightboxLogo'"), 'Docs page loader uses the hardened docs asset endpoint for media demos');
  context.assert(!routeController.includes('src="/XTend-Logo.png"'), 'Docs page loader avoids root-relative parent asset URLs in demos');
  context.assert(routeController.includes('resolveDocsToastApi'), 'Docs page loader routes toast actions through the framework toast API');
  context.assert(routeController.includes('xtend-docs-toast-dropped'), 'Docs page loader emits a diagnostic when the framework toast API is unavailable');
  context.assert(!routeController.includes('XTend Toast Middleware'), 'Docs page loader no longer owns a custom toast middleware');
  context.assert(!routeController.includes("document.createElement('x-toast')"), 'Docs page loader does not duplicate native x-toast container creation');
  context.assert(!routeController.includes('container.style.right'), 'Docs page loader does not hard-code toast viewport positioning');
  context.assert(!routeController.includes('window.showToast'), 'Docs page loader avoids legacy global toast helper paths');
  context.assert(routeController.includes("template.setAttribute('data-x-code-mode'"), 'Docs demo code blocks mark template content mode');
  context.assert(routeController.includes('template.content.appendChild(document.createTextNode(snippetCode))'), 'Docs demo RMT snippets preserve plain text template content');
  context.assert(routeController.includes('ensureDocsDemoScaffold'), 'Docs page loader repairs incomplete RMT demo slots before rendering snippets');
  context.assert(routeController.includes('hydrateDocsCodeBlocks(shell.demoSlot'), 'Docs page loader hydrates x-code demo blocks after each route render');
  context.assert(routeController.includes('window.XTendLoader.hydrateTree'), 'Docs page loader delegates dynamic component hydration to the XTend loader');
  context.assert(routeController.includes('hydrateDocsComponentPreview(preview, demo') && routeController.includes("schema: 'xtend.docs.component-demo-hydration.v1'"), 'Docs component demo islands hydrate their trigger and target nodes through an observable RMT contract');
  context.assert(routeController.includes('xtend-docs-code-hydrated'), 'Docs page loader emits a code hydration route diagnostic event');
  ...
  context.assert(!routeController.includes('._render('), 'Docs page loader avoids private x-code render monkeypatches');
  context.assert(routeController.includes("pack: 'core'"), 'Docs page loader uses the bundled core icon pack for shell icon actions');
  context.assert(routeController.includes('renderRmtDomTemplate'), 'Docs page loader renders RMT DOM descriptors');
  context.assert(routeController.includes('data-rmt-shell'), 'Docs page loader marks the RMT shell');
  context.assert(routeController.includes('shellFirst'), 'Docs page loader exposes shell-first render metadata');
  context.assert(routeController.includes('data-rmt-template'), 'Docs page loader marks rendered template');
  context.assert(routeController.includes('data-rmt-parse-schedule'), 'Docs page loader marks Parsedown parse schedule');
  context.assert(routeController.includes('normalizeDocsParsedownCodeEntities'), 'Docs page loader normalizes Parsedown inline code entity output');
  context.assert(routeController.includes('normalizedCodeEntityCount'), 'Docs page loader exposes Parsedown code normalization diagnostics');
  context.assert(routeController.includes('#md-content hr'), 'Docs page loader styles semantic Parsedown horizontal rules');
  context.assert(routeController.includes('data-rmt-trust-boundary'), 'Docs page loader marks Trusted DOM boundary');
  context.assert(routeController.includes('xtendDocsRmtLastRender'), 'Docs page loader exposes last RMT render metadata');
  context.assert(routeController.includes('updateRoute(context = {})'), 'Docs page component supports XRouter route reuse updates');
  context.assert(routeController.includes('DOCS_ROUTE_CONTENT_CACHE'), 'Docs page loader caches sanitized route content');
  context.assert(routeController.includes('scheduleDocsAfterPaint'), 'Docs page loader moves secondary route work after paint');
  context.assert(routeController.includes('scheduleDocsIdle'), 'Docs page loader moves demo hydration into idle work');
  context.assert(routeController.includes('xtend-docs-lane-complete'), 'Docs page loader emits per-lane route telemetry');
  context.assert(routeController.includes('data-rmt-document-title'), 'Docs page loader mirrors RMT document title metadata on the shell');
