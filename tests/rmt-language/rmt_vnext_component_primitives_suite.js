...
async function runRmtVNextComponentPrimitivesSuite(options = {}) {
  ...
  context.assert(matrix.manifestCount === 42, 'matrix covers all public manifest entries');
  context.assert(matrix.publicComponentCount === 38, 'matrix classifies renderable public UI components');
  context.assert(matrix.nonVisualCount === 4, 'matrix classifies module/demo/utility entries outside normal DOM rendering');
  context.assert(matrix.withRmtMetadata === 40, 'matrix preserves existing RMT metadata coverage');
  context.assert(matrix.withComponentContract === 38, 'matrix preserves existing Component Contract coverage');
