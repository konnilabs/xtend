...
function runXtendLoaderSkeletonProfilesSuite(options = {}) {
  ...
  context.assert(loader.includes('function skeletonTargetHasContent') && loader.includes('normalizeSkeletonLayoutMode(options.layoutMode)') && loader.includes("target.setAttribute('data-xtend-skeleton-mode', mode)") && loader.includes("target.setAttribute('data-xtend-skeleton-cache', 'overlay')") && loader.includes("target.removeAttribute('data-xtend-skeleton-mode')"), 'loader overlays skeletons over committed content, accepts an owner hint and retains inert geometry');
  context.assert(loader.includes('@media (prefers-reduced-motion: reduce)'), 'loader preserves reduced-motion geometry policy');
  context.assert(types.includes('responsive?:') && types.includes('breakpoint?: string') && types.includes("layoutMode?: 'auto' | 'flow' | 'overlay'"), 'loader types expose responsive descriptors and layout ownership');
...
function runDocsShellCatfoodingSuite(options = {}) {
  ...
  context.assert(menu.length === 169, 'menu keeps 169 canonical bilingual articles');
  ...
  ['de', 'en'].forEach((locale) => {
    ...
    context.assert(compact.schema === 'xtend.docs.search-index.v1' && compact.entryCount === 169, `${locale} compact index has contract and full inventory`);
    context.assert(fulltext.schema === 'xtend.docs.search-fulltext-index.v1' && fulltext.entryCount === 169, `${locale} fulltext index has contract and full inventory`);
  ...
  const searchResultStyles = indexPhp.slice(indexPhp.indexOf('.docs-search-result::part(link)'), indexPhp.indexOf('.docs-search-status[hidden]'));
  context.assert(searchResultStyles.includes('--xtend-link-current-indicator: transparent;') && searchResultStyles.includes('--link-active-decoration: none;') && searchResultStyles.includes('box-shadow: none;'), 'Docs search keeps one outer highlight rail without an inner x-link indicator');
  ...
  context.assert(browserSmoke.includes('activateHighlightedSearchResult') && browserSmoke.includes("entry?.command?.command === 'docs.route.navigate'") && browserSmoke.includes("enterNavigation.inputSource === 'keyboard'") && browserSmoke.includes("enterNavigation.selectedActive && enterNavigation.internalBoxShadow === 'none'"), 'ChromeDriver shell smoke verifies keyboard Enter telemetry and a single indicator on the active search result');
