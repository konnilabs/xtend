...
function runXtendLoaderSkeletonProfilesSuite(options = {}) {
  ...
  const generatedRouterRecord = (docsBundleReport.bundleFiles || []).find((entry) =>
    /(?:^|\/)x-router-[^/]+\.mjs$/u.test(entry.path || entry.fileName || '')
    || (entry.modules || []).some((module) => /(?:^|\/)components\/xrouter\.js$/u.test(module.id || ''))
  );
...
function runDocsShellCatfoodingSuite(options = {}) {
  ...
  context.assert(indexPhp.includes('x-link,x-input,x-form,x-header,x-hero,x-footer') && !indexPhp.includes('x-hero,x-router,x-footer'), 'Docs preload keeps XLink available without racing Maraca for XRouter ownership during resume');
