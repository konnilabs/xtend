...
async function activateDocsPlaygroundIsland({ root, locale, relatedLinks, signal }) {
  const { createPlaygroundIsland } = await import('./playground-island.mjs');
  if (signal?.aborted) return null;
  const controller = createPlaygroundIsland({
    root,
    locale,
    relatedLinks,
    signal,
    prepare: prepareDocsRmtPlaygroundLayoutElements,
    render: renderDocsRmtPlayground
  });
  await controller.ready;
  return () => controller.dispose();
}

export class XtendDocPage extends HTMLElement {
  constructor() {
    super();
  ...
  resumeInitialRoute(context = {}) {
    ...
    if (slug === 'learn-rmt-playground') {
      const playgroundDisposer = scheduleDocsVisibleOrIntentIsland(shell.mdContent, async () => {
        ...
        if (!this.isActiveRouteToken(token)) return null;
        return activateDocsPlaygroundIsland({
          root: shell.mdContent,
          locale,
          relatedLinks: [],
          signal: this.__xtendDocsRouteAbortController?.signal
        });
      }, { isActive: () => this.isActiveRouteToken(token) });
  ...
  renderRoute(context = {}) {
    ...
    const completeParsedownCommit = () => {
      ...
      commitParsedownContent().then(async (committed) => {
        ...
        if (slug === 'learn-rmt-playground') {
          const playgroundDisposer = scheduleDocsVisibleOrIntentIsland(shell.mdContent, () =>
            activateDocsPlaygroundIsland({
              root: shell.mdContent,
              locale,
              relatedLinks,
...
export function createRouteController({ ElementClass = XtendDocPage } = {}) {
  let disposed = false;
  return Object.freeze({ ElementClass, dispose() { if (disposed) return; disposed = true; } });
}
