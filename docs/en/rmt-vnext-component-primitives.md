...
How RMT describes components and XTend UI renders that description.

## What it covers

RMT Component Primitives and XTend UI describes the public RMT surface for this page: which records are involved, which adapters exercise them and which scheduler signals a host should verify.
...
Start RMT Component Primitives and XTend UI with the smallest record example, validate it with the linter and only then attach adapters for host data, routing or components.
...
RMT Component Primitives and XTend UI is the public RMT runtime contract for `docs/en/rmt-vnext-component-primitives.md`. The stable signal is not article length; it is whether an external host can verify the named files, names and checks without private project knowledge.
...
Sources:
- `docs/en/rmt-vnext-component-primitives.md`
- `docs/menu.json`
- `package.json`
