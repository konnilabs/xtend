...
An additional diagnostic artifact contains phase, worker, runtime and npm logs, plus the last 32 KiB from the local capability driver. Failures before browser session creation remain diagnosable. npm downloads use an absolute cache path under the runner temporary directory. Cache configuration requires no npm invocation; `actions/cache` restores downloads before `setup-node` without invoking its still unpinned npm version. The two optional browser/network branches retain explicit opt-in inputs and have bounded execution times. Manual acceptance must enable both inputs to exercise these branches as well.

## Observe Maraca 0.8.0

DEV Surface may explicitly request current snapshots after a lightweight DEV API signal. This does not require a permanent `runtime.subscribe()`. Compare shell response with the tool connected and disconnected, including attachment after boot. Panic and recovery must remain readable on the next explicit request in both cases; unredacted payloads do not belong in diagnostics. The [DEV API contract](./xtend-dev-api.md) covers subscription cleanup and synchronous reads.
