...
ECH-WP-08 documents the form-control tokens for `x-checkbox` as a public theme contract. `signatureDesign` stays a clear checkbox with visible check mark, label, hint and error hierarchy. Density-Profile: `comfortable`, `compact`, `dense`. Invalid: error states are mirrored through text, error surface, border, focus ring and ARIA.
