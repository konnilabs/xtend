...
Use [RMT vNext Authoring](./rmt-vnext-authoring.md) as the reference for complete language details. Read the [App DSL](./xtendrmt-app-dsl.md) when you want to model full applications, then continue with the [Runtime Bridge](./xtendrmt-runtime-bridge.md), [RMT Linter](./rmt-linter.md) and [Language Server](./rmt-language-server.md).
