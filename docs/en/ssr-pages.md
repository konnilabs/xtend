...
`createPageClient({ initialPage, viewTransitions: true })` enables browser View Transitions for page visits. `startMaracaPageApplication()` forwards the same option. Background reloads do not animate; unsupported browsers and reduced motion use the regular update. Visits can opt out with `{ transition: false }`. A custom `transition: async update => { /* … */ await update(); }` host callback remains available and takes precedence; `{ transition: true }` activates it.
