...
- Keine Runtime-Aenderung an `components/prism.js`, `components/turndown.js` oder `design-tokens/xtend-design-tokens.js`.
...
Der Gate prueft, dass `./design-tokens` eine `types`-Condition besitzt, dass die Komponenten-Vendor-Dateien eigene `.d.ts`-Facades haben, dass keine Component-`.js`-Datei ohne Declaration-Gap bleibt und dass die Facades keine Runtime-Imports oder Vendor-Implementierungsdetails kopieren.
