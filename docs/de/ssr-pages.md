...
`createPageClient({ initialPage, viewTransitions: true })` aktiviert Seitenanimationen über die bestehende RMT AnimationEngine und XUtils. `data-xtend-page-transition` markiert den animierten Inhalt innerhalb des Seitenroots; eine persistente Shell bleibt dadurch ruhig. `startMaracaPageApplication()` reicht dieselbe Option weiter. Hintergrundabfragen animieren nicht; fehlende Animationsunterstützung, fehlende XUtils und reduzierte Bewegung verwenden die reguläre Aktualisierung. Die Navigation wartet nicht auf die Animation; ein neuer Besuch oder Dispose beendet laufende Effekte. Ein Besuch kann mit `{ transition: false }` aussteigen. Eine eigene `transition: async update => { /* … */ await update(); }`-Hostfunktion bleibt verfügbar und hat Vorrang; sie wird mit `{ transition: true }` aktiviert.
