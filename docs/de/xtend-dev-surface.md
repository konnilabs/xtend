...
## Maraca 0.8.0 beobachten

DEV Surface darf nach einem schlanken DEV-API-Signal ausdrücklich aktuelle Snapshots abrufen. Ein dauerhaftes `runtime.subscribe()` ist dafür nicht nötig. Vergleiche die Shell-Reaktion mit verbundenem und getrenntem Werkzeug und prüfe auch eine Verbindung nach dem Boot. Panic und Recovery müssen in beiden Fällen beim nächsten Einzelabruf lesbar bleiben; unredigierte Payloads gehören nicht in die Diagnoseansicht. Der [DEV-API-Vertrag](./xtend-dev-api.md) beschreibt Subscription-Cleanup und synchrone Abrufe.
